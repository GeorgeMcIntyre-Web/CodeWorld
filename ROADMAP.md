...
- **Goal:** Automate critical user flows so regressions are caught before deploy.
- **In scope:** Add Playwright (or Cypress); at least one E2E test (e.g. open app, go to Playground, paste text, assert word count; or Settings theme toggle). Run E2E in CI on push/PR (optional: only on `main` or nightly).
- **Out of scope:** Full visual regression suite; testing third-party auth UI in E2E.

### 2. Full auth (Cloudflare or OAuth)
...
### 5. Docs, accessibility, and polish
